# Chordex MVP – User Stories

User stories derived from [mvp-desc-scope.md](../mvp-desc-scope.md). Each story has a status at the top: `TODO` (not implemented) or `Done` (implemented).

## Architecture

| Story | Status |
|-------|--------|
| [Firebase setup](./architecture/firebase-setup.md) | TODO |
| [Offline PWA](./architecture/offline-pwa.md) | TODO |
| [Data model](./architecture/data-model.md) | TODO |

## Song

| Story | Status |
|-------|--------|
| [Create song](./song/song-create.md) | Done |
| [Edit song](./song/song-edit.md) | Done |
| [Live preview](./song/song-live-preview.md) | Done |
| [Search songs](./song/song-search.md) | Done |
| [Tag songs](./song/song-tag.md) | Done |
| [Delete song](./song/song-delete.md) | Done |
| [Duplicate song](./song/song-duplicate.md) | Done |

## Setlist

| Story | Status |
|-------|--------|
| [Create setlist](./setlist/setlist-create.md) | Done |
| [Add songs to setlist](./setlist/setlist-add-songs.md) | Done |
| [Reorder songs](./setlist/setlist-reorder.md) | Done |
| [Setlist notes](./setlist/setlist-notes.md) | Done |
| [Setlist player mode](./setlist/setlist-player-mode.md) | Done |

## Player

| Story | Status |
|-------|--------|
| [Player mode](./player/player-mode.md) | TODO |

## Sharing

| Story | Status |
|-------|--------|
| [Share song via link](./sharing/share-song-link.md) | TODO |
| [Share setlist via link](./sharing/share-setlist-link.md) | TODO |
| [Open shared song](./sharing/shared-song-open.md) | TODO |
| [Save shared song to My Songs](./sharing/shared-song-save.md) | TODO |
| [Duplicate prevention](./sharing/shared-song-duplicate-prevention.md) | TODO |

## Export

| Story | Status |
|-------|--------|
| [Export library](./export/export-library.md) | TODO |

## Security

| Story | Status |
|-------|--------|
| [Security rules](./security/security-rules.md) | TODO |
